...
let platform = {
  ...
  "spotify": "spsearch",
  "deezer": "dzsearch",
}, arr = ["youtube", "youtube music", "soundcloud", "spotify", "deezer"];
class Blue extends EventEmitter {
  ...
  init(client){
   ...
   this._nodes.forEach((node)=>{
     ...
     client.on("voiceStateUpdate", async (oS, nS) => {
       ...
       if (nS.id == client.user.id && nS?.channelId !== oS?.channelId) {
         if (player) {
           if (player.state.connect) {
             if (!player.playing || !player.queue.current) {
               player.connect({ guildId: oS.guild.id, voiceChannel: nS?.channelId });
             } else player.setVoiceChannel(nS?.channelId);
           }
         }
       }
     });
  ...
  sendGuildShardData(data) {
    ...
    return new Promise(async(resolve, reject) => {
      if(!this.client) return reject(data);
     const guild = await this.client.guilds.fetch(data.d.guild_id).catch(()=>null);
     if(guild){
     resolve(guild);
       this.client.ws.shards.get(guild.shardId).send(data);
     } else {
      reject(guild);
     }
    });
  } catch(e){
    throw new Error("blue.js :: Unable to send data to the guild.");
  }
  }
  ...
  async search(param, requester) {
       ...
       if(typeof param == "object"){
        ...
        query = param.query;
           if(spotify.test(param.query)){
             ...
             if(type == "playlist"){
           const sp = await this.spotify.search(query);
           return sp;
         }
         if(type == "album"){
          const sp = await this.spotify.search(query);
    ...
    if (/(?:https?|ftp):\/\/[\n\S]+/gi.test(query)) {
      ...
      if (!result || result?.tracks?.length === 0){ 
       ...
       result.tracks.map((songs) => {
        songs.info.requester = requester;
      })
      return result;
    } else {
      ...
      handleEvents(payload){
        if (!payload.guildId) throw new Error(`Node#event unknown event received.`);
    ...
    const type = payload.type;
      ...
      const error = new Error(`blue.js :: unknown node event '${type}'.`);
