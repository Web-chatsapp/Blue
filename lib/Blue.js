...
class Blue extends EventEmitter {
  ...
  init(client){
   ...
   sendGuildShardData({options: data}){
    try{
    if(!data) throw new Error("Parameter of 'sendGuildShardData' must be present.")
    return new Promise(async(resolve, reject) => {
      if(!this.client) return reject(guild);
     const guild = await this.client.guilds.fetch(data.d.guild_id);
     if(guild){
     resolve(guild);
  ...
  create({options: data}) {
    let { guildId, voiceChannel, textChannel } = data;
    if (!guildId)
    ...
    if (/(?:https?|ftp):\/\/[\n\S]+/gi.test(query)) {
      ...
      handleEvents(payload){
        if (!payload.guildId) throw new Error(`Node#event unknown event '${type}'.`);
