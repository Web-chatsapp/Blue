...
class BluePlayerManager extends EventEmitter {
  constructor(blue, node, options){
    ...
    try {
      this.isPlaying = true;
      this.position = 0;
//console.log("fine "+this.guildId, this.currentTrack.track)
   //   let conn = this.join(this.blue.client, this.guildId, this.voiceChannel)
  ...
  voiceChannelJoin(this.blue.client.token, guildId, voiceChannel)
